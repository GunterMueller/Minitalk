...
void codeExpression(Node *expression, Boolean valueNeeded)
{
  ...
  switch (expression->type) {
	...
	if (!expression->all.message.superFlag) {
	  code2(OP_SEND,
		numberArguments,
		makeLiteral(expression->all.message.selector),
		-numberArguments);
	} else {
	  code2(OP_SENDSUPER,
		numberArguments,
...
void codeLiteral(Node *literalNode)
{
  ...
  switch (literalNode->type) {
	...
	machine.compilerLiteral =
	  newSymbol(literalNode->all.symbol.name);
	break;
    case N_INTNUM:
	machine.compilerLiteral =
	  newSmallInteger(literalNode->all.intnum.value);
	break;
    case N_FLONUM:
	machine.compilerLiteral =
	  newFloat(literalNode->all.flonum.value);
	break;
    case N_STRING:
	machine.compilerLiteral =
	  newString(literalNode->all.string.value);
	break;
    case N_CHARCON:
	machine.compilerLiteral =
	  newCharacter(literalNode->all.charcon.value);
	break;
    case N_ARRAY:
	...
	object =
	  allocateObject(getPointer(machine.Array,
				    VALUE_IN_ASSOCIATION),
			 literalNode->all.array.numberElements,
			 TRUE);
